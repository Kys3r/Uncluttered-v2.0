...
Passport.use(new GoogleStrategy({
		...
		callbackURL: process.env.DOMAIN + '/google-auth/auth/callback'
	},
	(accessToken, refreshToken, profile, cb) => {
		if (Validate.isDefined(profile.emails) && !Validate.isEmpty(profile.emails))
		{
			Models.Users.findOrCreate({
				defaults: { email: profile.emails[0].value, verified: true, google_id: profile.id, profil_picture: "PP-" + String(Math.floor(Math.random() * 5) + 1) + ".png" },
			...
			.spread(async (user, created) =>
			{
				...
				{
					await Models.Users.update({ google_id: profile.id, verified: true }, { where: {email: profile.emails[0].value} })
					cb(null, user)
				}
			})
		}
...
Router.get('/auth/callback', Passport.authenticate('google', { failureRedirect: '/google-auth/failure-redirect' }), async (req, res) =>
{
